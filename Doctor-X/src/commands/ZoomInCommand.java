...
public class ZoomInCommand implements Command
{
	...
	public ZoomInCommand(Application application, View view)
	{
	...
	public void execute(AtomComponent element)
	{
		...
		if(zoomfactor < 1.5)
		{
			...
			zoomfactor = model.getDocuments().get(canvasId).zoomFactor;
			...
			/*view.centralPart.canvasList.get(canvasId).setPreferredSize(new Dimension(
					(int)Math.round(view.centralPart.canvasList.get(canvasId).getPreferredSize().height*zoomfactor),
					(int)Math.round(view.centralPart.canvasList.get(canvasId).getPreferredSize().width*zoomfactor)));*/
