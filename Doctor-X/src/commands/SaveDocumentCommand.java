...
public class SaveDocumentCommand implements Command
{
	...
	public SaveDocumentCommand(Application model, View view)
	{
		this.model = model;
		this.view = view;
	}
	...
	{		
		if(view.centralPart.getSelectedIndex() != -1)
		{
			...
			//provjera da li se prvi put snima
			if(Application.documents.get(canvasId).location.length() > 2)
			{
				writer.writeDocumentParametars(Application.documents.get(canvasId));
			}
			else
			{
